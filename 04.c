...
solutions Solve(FILE* input)
{
	...
	static range elves[2];
	while (fscanf(input, "%d-%d,%d-%d\n", &elves[0].start, &elves[0].end, &elves[1].start, &elves[1].end) != EOF) {
		// Expressions return 1 when true, this allows for some fun shortcuts.
		r.a += (elves[0].start <= elves[1].start && elves[0].end >= elves[1].end) || (elves[1].start <= elves[0].start && elves[1].end >= elves[0].end);
		r.b += (elves[0].end >= elves[1].start && elves[1].end >= elves[0].start);
	}
