...
solutions Solve(FILE* input)
{
	...
	static char line[7]; // (4-5 Digits) + \n + \0
	while (fgets(line, sizeof (line), input) != NULL) {
		if (line[0] == '\n') { // Elf calories summed, evaluate.
			if (current > top[0]) {
				memmove(&top[1], &top[0], sizeof (int) * 2);
				top[0] = current;
			} else if (current > top[1]) {
				memmove(&top[2], &top[1], sizeof (int));
				top[1] = current;
			} else if (current > top[2]) top[2] = current;
			current = 0; // Reset for next Elf.
		} else current += atoi(line);
	}

	solutions r = {top[0], top[0] + top[1] + top[2]};
	return r;
}
