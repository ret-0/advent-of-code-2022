...
int GetPriority(char c) { return (islower(c)) ? c - 96 : c - 38; }
...
solutions Solve(FILE* input)
{
	...
	while (fgets(line, sizeof (line), input) != NULL) {
		int half = (strlen(line) - 1) / 2; // (Length of characters (not including \0) - \n) / 2
		...
		second[half] = 0x00;
		// Copy.
		memcpy(first, line, half);
		memcpy(second, line + half, half);

		char* p = first;
		while (*p) {
			if (strchr(second, *p) != NULL) {
				sum += GetPriority(*p);
				break;
			}
			p++;
		...
		// Save last three lines.
		memcpy(lines[i], line, sizeof (line));
		i++;
		if (i == 3) {
			p = lines[0];
			while (*p) {
				if (strchr(lines[0], *p) != NULL && strchr(lines[1], *p) != NULL && strchr(lines[2], *p) != NULL) {
					groupSum += GetPriority(*p);
					break;
				}
				p++;
			}
